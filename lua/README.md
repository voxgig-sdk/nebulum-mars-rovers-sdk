# NebulumMarsRovers Lua SDK



The Lua SDK for the NebulumMarsRovers API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Photo()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── nebulum-mars-rovers_sdk.lua    -- Main SDK module
...
The main module (`nebulum-mars-rovers_sdk`) exports the SDK constructor
