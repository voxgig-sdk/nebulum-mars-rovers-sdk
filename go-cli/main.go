// Command NebulumMarsRovers-cli is a CLI + REPL for the NebulumMarsRovers SDK,
...
const prompt = "nebulum-mars-rovers"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "photo"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NEBULUM_MARS_ROVERS_APIKEY (api key), NEBULUM_MARS_ROVERS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NEBULUM_MARS_ROVERS_APIKEY carries the API key and
	// NEBULUM_MARS_ROVERS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NEBULUM_MARS_ROVERS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NEBULUM_MARS_ROVERS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNebulumMarsRoversSDK(opts)
