# nebulum-mars-rovers-cli

boru-driven command-line client **and** interactive REPL for the NebulumMarsRovers
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nebulum-mars-rovers-cli)
make build

# 2. See usage (words, entities, env vars)
./nebulum-mars-rovers-cli --help

# 3. Provide credentials once, via the environment
export NEBULUM_MARS_ROVERS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nebulum-mars-rovers-cli list photo
./nebulum-mars-rovers-cli load 1 photo            # {id:1} shorthand
./nebulum-mars-rovers-cli load '{id:1}' photo       # explicit match map

# 5. Override the API base URL for a single call
NEBULUM_MARS_ROVERS_BASE=https://api.example.com ./nebulum-mars-rovers-cli list photo

# 6. No arguments -> interactive REPL
./nebulum-mars-rovers-cli
nebulum-mars-rovers> list photo
nebulum-mars-rovers> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nebulum-mars-rovers-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nebulum-mars-rovers-cli list photo
   ```
...
```sh
./nebulum-mars-rovers-cli load 1 photo          # scalar shorthand for {id:1}
./nebulum-mars-rovers-cli load '{id:1}' photo     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `nebulum-mars-rovers>`). Each line is
evaluated as its own boru expression:

```text
$ ./nebulum-mars-rovers-cli
nebulum-mars-rovers> list photo
nebulum-mars-rovers> /help
nebulum-mars-rovers> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `photo`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nebulum-mars-rovers-cli`. |
...
(`./nebulum-mars-rovers-cli <expr>`) and interactively (the REPL), and expressions compose the
