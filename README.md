...
Access real-time photos taken by Mars rovers Perseverance and Curiosity through NASA missions.

Learn more about Nebulum Mars Rovers API at [rovers.nebulum.one](https://rovers.nebulum.one).
...
$photos = $client->Photo()->list();
print_r(array_map(fn($item) => $item->data_get(), $photos));
