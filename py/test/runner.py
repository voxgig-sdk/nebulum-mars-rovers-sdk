# NebulumMarsRovers SDK test runner
...
class NebulumMarsRoversTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NebulumMarsRoversTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NebulumMarsRoversTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NebulumMarsRoversTestRunner.getenv("NEBULUM_MARS_ROVERS_TEST_LIVE")
        override = NebulumMarsRoversTestRunner.getenv("NEBULUM_MARS_ROVERS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NebulumMarsRoversTestRunner.getenv(key)
        ...
        explain = NebulumMarsRoversTestRunner.getenv("NEBULUM_MARS_ROVERS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEBULUM_MARS_ROVERS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NebulumMarsRoversTestRunner._test_control is not None:
            return NebulumMarsRoversTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NebulumMarsRoversTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NebulumMarsRoversTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NebulumMarsRoversTestRunner.load_test_control()
...
def load_env_local():
    NebulumMarsRoversTestRunner.load_env_local()


def env_override(m):
    return NebulumMarsRoversTestRunner.env_override(m)


def entity_data(v):
    return NebulumMarsRoversTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NebulumMarsRoversTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NebulumMarsRoversTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NebulumMarsRoversTestRunner.load_test_control()


def live_client_options():
    return NebulumMarsRoversTestRunner.live_client_options()


def live_delay_ms():
    return NebulumMarsRoversTestRunner.live_delay_ms()
