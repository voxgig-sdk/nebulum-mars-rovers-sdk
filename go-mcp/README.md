# nebulum-mars-rovers-mcp

[MCP](https://modelcontextprotocol.io) server exposing the NebulumMarsRovers SDK as
two agent tools — `nebulum-mars-rovers_list` and `nebulum-mars-rovers_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nebulum-mars-rovers-mcp)
make build

# 2. Provide credentials via the environment
export NEBULUM_MARS_ROVERS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nebulum-mars-rovers \
  -- /absolute/path/to/nebulum-mars-rovers-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nebulum-mars-rovers-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nebulum-mars-rovers_list: first page of records
{ "entity": "photo" }
{ "entity": "photo", "query": { } }

// nebulum-mars-rovers_load: one record by id
{ "entity": "photo", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nebulum-mars-rovers-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nebulum-mars-rovers \
     -- "$PWD"/dist/*/nebulum-mars-rovers-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nebulum-mars-rovers_list` and `nebulum-mars-rovers_load` tools now appear
   in new sessions. Ask the agent to *"list photo using nebulum-mars-rovers"*
   and it calls `nebulum-mars-rovers_list` with `{"entity":"photo"}`.
...
```sh
export NEBULUM_MARS_ROVERS_APIKEY=sk_live_xxx            # API key
export NEBULUM_MARS_ROVERS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nebulum-mars-rovers_list` tool
...
### Call the `nebulum-mars-rovers_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nebulum-mars-rovers_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nebulum-mars-rovers_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEBULUM_MARS_ROVERS_APIKEY` | API key sent with every request. |
| `NEBULUM_MARS_ROVERS_BASE` | Optional override of the API base URL. |
...
```sh
./nebulum-mars-rovers-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nebulum-mars-rovers_load","arguments":{"entity":"photo","query":{"id":1}}}}'
```
