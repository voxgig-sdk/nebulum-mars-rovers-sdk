# NebulumMarsRovers SDK Photo entity
...
class PhotoEntity
  ...
  def data_set(args)
    if args
      @_data = NebulumMarsRoversHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = NebulumMarsRoversHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Photo, Hash] the loaded Photo; raises NebulumMarsRoversError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NebulumMarsRoversHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Photo>, Array] the matching Photo items; raises NebulumMarsRoversError on failure
