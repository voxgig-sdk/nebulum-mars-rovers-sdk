...
require_relative 'NebulumMarsRovers_types'
...
class NebulumMarsRoversSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NebulumMarsRoversHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NebulumMarsRoversHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NebulumMarsRoversFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NebulumMarsRoversUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NebulumMarsRoversHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NebulumMarsRoversHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NebulumMarsRoversHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => NebulumMarsRoversError.new(
        "#{op}_allow",
        "NebulumMarsRoversSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NebulumMarsRoversError => err
    ...
    if fetched.is_a?(Hash)
      status = NebulumMarsRoversHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NebulumMarsRoversError.new(
        "graphql_error", "NebulumMarsRoversSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NebulumMarsRoversSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
