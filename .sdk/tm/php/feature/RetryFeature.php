...
// NebulumMarsRovers SDK retry feature
...
class NebulumMarsRoversRetryFeature extends NebulumMarsRoversBaseFeature
{
    ...
    public function init(NebulumMarsRoversContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NebulumMarsRoversContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NebulumMarsRoversContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NebulumMarsRoversContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
