...
// NebulumMarsRovers SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NebulumMarsRoversOmni::makeStructRunner(
    ...
    {
        return NebulumMarsRoversOmni::objmodel(NebulumMarsRoversOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return NebulumMarsRoversOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return NebulumMarsRoversOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NebulumMarsRoversOmni::structfix($now, Struct::undef()))
                        !== json_encode(NebulumMarsRoversOmni::structfix($val, Struct::undef()))) {
