...
module NebulumMarsRoversUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NebulumMarsRoversResponse.new({})
    ...
    if fetch_err
      ...
      response = NebulumMarsRoversResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NebulumMarsRoversResponse.new(fetched)
    else
