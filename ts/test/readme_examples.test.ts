...
//     (\`new NebulumMarsRoversSDK(...)\` / \`NebulumMarsRoversSDK.test(...)\` become
//     \`NebulumMarsRoversSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NebulumMarsRoversSDK'
...
const TEST_SEED = {"entity":{"photo":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NebulumMarsRoversSDK(...)\` and any
// \`NebulumMarsRoversSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NebulumMarsRoversSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NebulumMarsRoversSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
