# NebulumMarsRovers TypeScript SDK



The TypeScript SDK for the NebulumMarsRovers API — a type-safe, entity-oriented client with full async/await support.
...
`list()` resolves to an array of Photo objects — iterate it directly:
...
```ts
try {
  const photo = await client.Photo().load({ id: 'example_id' })
  console.log(photo)
} catch (err) {
...
const photo = await client.Photo().load({ id: 'test01' })
// photo is a bare entity populated with mock response data
console.log(photo)
```
...
| `tester(testopts?, sdkopts?)` | `NebulumMarsRoversSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `NebulumMarsRoversSDK.test(testopts?, sdkopts?)` | `NebulumMarsRoversSDK` | Create a test-mode client. |
...
| `client` | `client(): NebulumMarsRoversSDK` | Return the parent SDK client. |
...
nebulum-mars-rovers/
├── src/
│   ├── NebulumMarsRoversSDK.ts        # Main SDK class
