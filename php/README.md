# NebulumMarsRovers PHP SDK



The PHP SDK for the NebulumMarsRovers API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Photo()` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $photo));
```
...
### NebulumMarsRoversSDK

```php
require_once 'nebulummarsrovers_sdk.php';
$client = new NebulumMarsRoversSDK($options);
```
...
```php
$client = NebulumMarsRoversSDK::test($testopts, $sdkopts);
```
...
```
php/
├── nebulummarsrovers_sdk.php          -- Main SDK class
...
The main class (`nebulummarsrovers_sdk.php`) exports the SDK class
