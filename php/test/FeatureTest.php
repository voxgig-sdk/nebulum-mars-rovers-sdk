...
// NebulumMarsRovers SDK feature test
//
...
class FtCtrl extends NebulumMarsRoversControl
{
...
class FtClient
{
    ...
    private NebulumMarsRoversUtility $utility;

    public function __construct(NebulumMarsRoversUtility $utility, array $options)
    {
    ...
    public function get_utility(): NebulumMarsRoversUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NebulumMarsRoversContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NebulumMarsRoversContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NebulumMarsRoversContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NebulumMarsRoversConfig::make_feature($name);
    ...
    private static function buildUrl(NebulumMarsRoversSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NebulumMarsRoversError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NebulumMarsRoversResponse($fetched);
        }
    ...
    private function fail(NebulumMarsRoversContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NebulumMarsRoversContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
