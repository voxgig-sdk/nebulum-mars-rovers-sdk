...
//    only - a typed NebulumMarsRoversContext there would make every ctx
...
final class NebulumMarsRoversOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NebulumMarsRoversContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NebulumMarsRoversSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NebulumMarsRoversResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NebulumMarsRoversError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NebulumMarsRoversResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NebulumMarsRoversError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NebulumMarsRoversError(is_string($code) ? $code : '', $msg);
    }
