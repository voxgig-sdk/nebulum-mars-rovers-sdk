...
func TestPhotoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		_, photoRef01ListOk := photoRef01ListResult.([]any)
		if !photoRef01ListOk {
...
func photoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NEBULUM_MARS_ROVERS_TEST_PHOTO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NEBULUM_MARS_ROVERS_TEST_PHOTO_ENTID"])
	...
	if env["NEBULUM_MARS_ROVERS_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewNebulumMarsRoversSDK(core.ToMapAny(mergedOpts))
	}
