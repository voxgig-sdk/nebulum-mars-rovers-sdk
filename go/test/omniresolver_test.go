...
import (
	...
	omni "github.com/voxgig-sdk/nebulum-mars-rovers-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NebulumMarsRoversError); is {
				out := map[string]any{
					"name":    "NebulumMarsRoversError",
...
func omniCtx(arg any, client *sdk.NebulumMarsRoversSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NebulumMarsRoversSDK); is {
			client = live
