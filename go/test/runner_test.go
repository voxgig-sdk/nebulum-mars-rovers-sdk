...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("NEBULUM_MARS_ROVERS_TEST_LIVE") == "TRUE" ||
		os.Getenv("NEBULUM_MARS_ROVERS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("NEBULUM_MARS_ROVERS_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.NebulumMarsRoversSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.NebulumMarsRoversSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.NebulumMarsRoversError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.NebulumMarsRoversSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.NebulumMarsRoversError{Msg: msg, Code: code}
}
