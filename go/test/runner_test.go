...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("NEBULUMMARSROVERS_TEST_LIVE") == "TRUE" ||
		os.Getenv("NEBULUMMARSROVERS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("NEBULUMMARSROVERS_TEST_EXPLAIN"); explain != "" {
